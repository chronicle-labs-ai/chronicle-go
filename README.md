# ChronicleLabs Go Library

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=https%3A%2F%2Fgithub.com%2Fchronicle-labs-ai%2Fchronicle-go)

The ChronicleLabs Go library provides convenient access to the ChronicleLabs APIs from Go.
...
A full reference for this library is available [here](https://github.com/chronicle-labs-ai/chronicle-go/blob/HEAD/./reference.md).
...
```go
response, err := client.Events.IngestEvent(...)
if err != nil {
...
```go
response, err := client.Events.WithRawResponse.IngestEvent(...)
if err != nil {
    return err
}
...
response, err := client.Events.IngestEvent(ctx, ...)
```
...
response, err := client.Events.IngestEvent(ctx, request, ...)
```
